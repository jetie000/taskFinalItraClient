...
function CollectionFields({ fields, setFields }: { fields: { id: number, name: string, type: string }[], setFields: Function }) {
        ...
        <>
            {fields.length > 0 && fields.map(field =>
                <div key={field.id} className="input-group mb-1">
                    ...
                    <input onChange={event => changeFieldName(event, field.id)} id={'fieldInput' + field.id} type="text" className="form-control w-25" placeholder='Введите название поля' />
                    <button className='btn btn-danger d-flex align-items-center' onClick={() => deleteField(field.id)}>
