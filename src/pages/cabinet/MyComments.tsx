...
function MyComments() {
    ...
    return ( 
        <div className='p-3 ps-5 pe-5 w-75'>
            {
                typeof(data) !== 'string' && data && data.length > 0 ?
                            ...
                            data.map(item =>
                                <li key={item.id} onClick={() => navigate('/collection/' + item.myCollection!.id + '/item/' + item.id)} className="list-group-item fs-4 d-flex gap-3 align-items-start cursor-pointer">
                                    ...
                                    <div className="d-flex align-items-center gap-2">
                                        ...
                                        <span>
                                            {item.likes?.filter(reaction => reaction.isLike === true).length || 0}
                                        </span>
                                        ...
                                        <span>
                                            {item.likes?.filter(reaction => reaction.isLike === false).length || 0}
                                        </span>
                    ...
                    :
                    <div className="fs-3 mt-2">Вы еще не оставляли комментариев</div>
            }
        </div>
     );
}
